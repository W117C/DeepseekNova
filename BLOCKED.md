...
- ~~执行账本（execution ledger）接线排期~~ **已裁决（2026-08-10 ADR）**：
  ...
  （只记录不恢复，IO 开销无对应价值）；(3) Authoritative 是大型功能开发
  （"一扇门"难逆）。复查触发条件：(a) DESIGN 路线图推进到 P5/P6 之后；
  (b) 出现真实崩溃恢复需求；(c) 1346 行代码产生维护负担（依赖升级
  break API）。撤销成本：低（删除 2 模块 + Cargo.toml 依赖）。
