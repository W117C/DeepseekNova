...
impl PromptBudgetController {
    pub fn evaluate_budget(&self, current_tokens: usize, proposed_addition: usize) -> BudgetDecision {
        if current_tokens + proposed_addition > self.max_total_tokens {
            if current_tokens > (self.max_total_tokens as f64 * 0.8) as usize {
                ...
                return BudgetDecision::Reject("Proposed addition drastically exceeds context window.".into());
            }
        }
