use reasonix_core::registry::Planner;
use reasonix_core::{
...
async fn run_plan_mode(
    ...
    while let Some(chunk) = stream.next().await {
        match chunk? {
            reasonix_core::chunk::Chunk::TextDelta(delta) => {
                plan_text.push_str(&delta);
                tx.send(Ok(RunEvent::TextDelta(delta))).await.ok();
            }
            reasonix_core::chunk::Chunk::ReasoningDelta { text, signature } => {
                tx.send(Ok(RunEvent::ReasoningDelta { text, signature }))
                    .await
                    .ok();
            }
            reasonix_core::chunk::Chunk::Usage(u) => {
                tx.send(Ok(RunEvent::Usage(u))).await.ok();
            }
            reasonix_core::chunk::Chunk::Done => {}
...
fn action_label(action: &reasonix_core::graph::Action) -> String {
    match action {
        reasonix_core::graph::Action::Think { .. } => "Think".to_string(),
        reasonix_core::graph::Action::CallTool { tool, .. } => format!("CallTool({tool})"),
        reasonix_core::graph::Action::Observe { .. } => "Observe".to_string(),
        reasonix_core::graph::Action::Reflect { .. } => "Reflect".to_string(),
        reasonix_core::graph::Action::Delegate { sub_agent, .. } => {
            format!("Delegate({sub_agent})")
        }
        reasonix_core::graph::Action::Parallel(_) => "Parallel".to_string(),
        reasonix_core::graph::Action::Conditional { .. } => "Conditional".to_string(),
    }
}
...
mod tests {
    ...
    use reasonix_core::graph::{Action, ExecutionNode};
