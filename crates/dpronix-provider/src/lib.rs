...
///
/// [vi]: dpronix_context::history::validate_replay_invariant
#[allow(clippy::manual_non_exhaustive)]
pub struct ValidatedRequest<'a> {
    pub messages: &'a [Message],
    pub tools: &'a [&'a dyn dpronix_core::Tool],
...
impl<'a> ValidatedRequest<'a> {
    ...
    pub fn new(
        messages: &'a [Message],
        tools: &'a [&'a dyn dpronix_core::Tool],
    ) -> Result<Self, Vec<String>> {
        dpronix_context::history::validate_replay_invariant(messages).map_err(|violations| {
            violations
                .iter()
                .map(|v| v.to_string())
                .collect::<Vec<String>>()
