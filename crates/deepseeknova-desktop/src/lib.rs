...
pub struct AppState {
    ...
    /// Session-cached MCP tools. Discovered once per session (spawning stdio
    /// server processes) and reused across prompts; cleared on new_session so
    /// the connections — and child processes — are released.
    pub session_mcp_tools:
        tokio::sync::Mutex<Option<Vec<std::sync::Arc<dyn deepseeknova_core::Tool>>>>,
...
pub fn run() {
    let app = tauri::Builder::default()
        ...
        .manage(AppState {
            ...
            session_mcp_tools: tokio::sync::Mutex::new(None),
