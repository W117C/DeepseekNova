...
/// (`.deepseeknova/skills/`). Every bundled `.md` must parse into a valid skill.
#[test]
fn e2e_bundled_superpowers_skills_all_parse() {
    // crate dir is <workspace>/crates/deepseeknova-skills; skills live at <workspace>/.deepseeknova/skills
    let bundled =
        std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../.deepseeknova/skills");
    if !bundled.exists() {
...
fn e2e_builtin_skills_load() {
    let skills = deepseeknova_skills::load_builtin_skills();
