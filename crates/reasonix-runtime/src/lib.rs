...
use reasonix_config::Config;
use reasonix_context::ContextProvider;
use reasonix_core::registry::RegistryHub;
use reasonix_core::runner::{RunEventStream, RunInput, Runner};
use reasonix_event::EventBus;
use reasonix_permission::{Decision, PermissionGate, Policy};
...
impl Runtime {
    ...
    pub async fn run(
        ...
        self.events
            .publish(reasonix_event::AgentEvent::ModelStarted {
    ...
    pub fn check_permission(&self, tool: &dyn reasonix_core::Tool, args: &str) -> Decision {
...
fn build_permission_gate(config: &Config) -> PermissionGate {
    ...
    for rule in &config.permissions.rules {
        let r = if let Some(ref subject) = rule.subject {
            reasonix_permission::Rule::with_subject(&rule.tool, subject)
        } else {
            reasonix_permission::Rule::new(&rule.tool)
        };

        match rule.mode {
            reasonix_config::PermissionMode::Allow => allow.push(r),
            reasonix_config::PermissionMode::Ask => ask.push(r),
            reasonix_config::PermissionMode::Deny => deny.push(r),
        }
    }

    let mode = match config.permissions.default_mode {
        reasonix_config::PermissionMode::Allow => Decision::Allow,
        reasonix_config::PermissionMode::Ask => Decision::Ask,
        reasonix_config::PermissionMode::Deny => Decision::Deny,
    };
...
mod tests {
    use super::*;
    use reasonix_config::Config;
    use reasonix_context::ContextEngine;
