...
#[test]
fn project_memory_loads_deepnova_md() {
    let dir = TempDir::new().unwrap();
    std::fs::write(dir.path().join("DPRONIX.md"), "# Project Context").unwrap();

    let mut pm = ProjectMemory::new();
    pm.load_deepnova_md(dir.path());

    assert!(pm.deepnova_md.is_some());
    assert!(pm.deepnova_md.unwrap().contains("Project Context"));
}

#[test]
fn project_memory_missing_deepnova_md_stays_none() {
    let dir = TempDir::new().unwrap();
    let mut pm = ProjectMemory::new();
    pm.load_deepnova_md(dir.path());
    assert!(pm.deepnova_md.is_none());
}
