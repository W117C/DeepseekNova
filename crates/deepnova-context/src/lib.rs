...
use deepnova_core::registry::Command;
use deepnova_core::types::{Message, Role, ToolSchema};
...
fn scan_dir(
    ...
    // Skip hidden directories except .git and .deepnova
    if let Some(name) = dir.file_name().and_then(|n| n.to_str()) {
        if name.starts_with('.') && name != "." && name != ".deepnova" {
            return Ok(());
        }
    }
...
impl PromptBuilder {
    ...
    pub fn build(
        ...
        if let Some(ref deepnova_md) = project_memory.deepnova_md {
            system_content.push_str("\n\n---\n## Project Context\n\n");
            system_content.push_str(deepnova_md);
        }
...
mod cache_tests {
    ...
    fn tool_schema_serialization_is_order_deterministic() {
        ...
        use deepnova_core::types::ToolSchema;
        let tools = vec![
            ToolSchema {
                name: "zebra".into(),
...
impl ProjectMemory {
    ...
    pub fn load_deepnova_md(&mut self, root: &Path) {
        let path = root.join("DPRONIX.md");
        if path.exists() {
            if let Ok(content) = std::fs::read_to_string(&path) {
                self.deepnova_md = Some(content);
            }
        }
    }

    /// Load all persistent memory entries from .deepnova/memory/*.md files.
    pub fn load_memory_files(&mut self, root: &Path) {
        let memory_dir = root.join(".deepnova").join("memory");
    ...
    /// Load custom slash commands from .deepnova/commands/*.md files.
...
impl ContextEngine {
    pub fn new(root: PathBuf) -> anyhow::Result<Self> {
        ...
        project_memory.load_deepnova_md(&root);
...
mod tests {
    ...
    fn prompt_builder_injects_project_memory() {
        ...
        pm.deepnova_md = Some("This is a Rust project.".into());
    ...
    fn workspace_scan_temp_dir() {
        let dir = std::env::temp_dir().join(format!("deepnova-ctx-test-{}", std::process::id()));
    ...
    fn workspace_scan_respects_gitignore() {
        let dir = std::env::temp_dir().join(format!("deepnova-ctx-gi-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
