...
async fn run_plan_mode(
    ...
    while let Some(chunk) = stream.next().await {
        match chunk? {
            deepseeknova_core::chunk::Chunk::TextDelta(delta) => {
                plan_text.push_str(&delta);
                tx.send(Ok(RunEvent::TextDelta(delta))).await.ok();
            }
            deepseeknova_core::chunk::Chunk::ReasoningDelta { text, signature } => {
                tx.send(Ok(RunEvent::ReasoningDelta { text, signature }))
                    .await
                    .ok();
            }
            deepseeknova_core::chunk::Chunk::Usage(u) => {
...
fn action_label(action: &deepseeknova_core::graph::Action) -> String {
    match action {
        deepseeknova_core::graph::Action::Think { .. } => "Think".to_string(),
        deepseeknova_core::graph::Action::CallTool { tool, .. } => format!("CallTool({tool})"),
        deepseeknova_core::graph::Action::Observe { .. } => "Observe".to_string(),
        deepseeknova_core::graph::Action::Reflect { .. } => "Reflect".to_string(),
        deepseeknova_core::graph::Action::Delegate { sub_agent, .. } => {
            format!("Delegate({sub_agent})")
        }
        deepseeknova_core::graph::Action::Parallel(_) => "Parallel".to_string(),
        deepseeknova_core::graph::Action::Conditional { .. } => "Conditional".to_string(),
    }
}
...
mod tests {
    ...
    use deepseeknova_core::graph::{Action, ExecutionNode};
    use deepseeknova_provider::Provider;
    use tokio_stream::StreamExt;
