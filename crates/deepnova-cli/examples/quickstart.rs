//! # DeepNova 快速上手示例
//!
//! 运行: `cargo run --example quickstart -p deepnova-cli`
//!
//! 展示 DeepNova 核心概念，无需 LLM API key：
...
fn main() -> anyhow::Result<()> {
    ...
    let resolved = deepnova_security::path::sanitize_path(&workspace, safe_path.to_str().unwrap())?;
    println!("   ✅ 安全路径: {}", resolved.display());

    let bad_result = deepnova_security::path::sanitize_path(&workspace, "../../etc/passwd");
    ...
    let limits = deepnova_security::limits::ResourceLimits::default();
    println!(
