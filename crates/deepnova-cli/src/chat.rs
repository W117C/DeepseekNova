...
use deepnova_provider::factory::ReasoningEffort;
    ...
    F: Fn(Option<ReasoningEffort>, Option<String>) -> anyhow::Result<Box<dyn Runner + Send>>,
{
    ...
    println!();
    println!("╭──────────────────────────────────────────────────╮");
    println!("│     deepnova — interactive chat                  │");
...
async fn handle_slash_command(
    ...
    match name {
        ...
        "skills" => {
            ...
            if !found {
                println!("No skills found. Create .md files in .deepnova/skills/");
            }
        }

        // ── MCP status ────────────────────────────────────────
        "mcp" => {
            println!("MCP servers are configured in deepnova.toml:");
            println!("  [[mcp_servers]]");
        ...
        "undo" => {
            ...
            println!("Use the checkpoint system: crates/deepnova-checkpoint");
        }
