...
use deepnova_agent::{CoordinatorRunner, PlanModeRunner};
use deepnova_core::planner::SimplePlanner;
use deepnova_core::runner::{RunInput, Runner};
use std::sync::Arc;
...
async fn main() -> anyhow::Result<()> {
    ...
    let config = deepnova_config::Config::load().unwrap_or_else(|e| {
        tracing::warn!("failed to load config, using defaults: {e}");
        deepnova_config::Config::default()
    });
    ...
    match &cli.command {
        ...
        Some(Commands::Run {
            ...
            if let Some(ref planner_model) = coordinator.planner_model {
                ...
                let executor_provider = resolve_provider_for_task(
                    &config,
                    &executor_model,
                    Some(deepnova_provider::factory::ReasoningEffort::High),
                )?;
        ...
        Some(Commands::Chat { model }) => {
            ...
            let baseline_effort = deepnova_provider::factory::resolve_effort(provider_cfg, None);

            loop {
                // Persistent session memory — shared across model/effort
                // rebuilds within the same `/new` session.
                let history: Arc<tokio::sync::Mutex<Vec<deepnova_core::Message>>> =
                    Arc::new(tokio::sync::Mutex::new(Vec::new()));

                let history_clone = Arc::clone(&history);
                let cfg = &config;
                let agent_factory =
                    move |effort: Option<deepnova_provider::factory::ReasoningEffort>,
        ...
        Some(Commands::Serve { addr }) => {
            ...
            let server = deepnova_serve::Server::new(runner);
...
fn resolve_provider_cfg<'a>(
    config: &'a deepnova_config::Config,
    model: Option<&str>,
) -> &'a deepnova_config::ProviderConfig {
...
fn resolve_provider(
    config: &deepnova_config::Config,
    model: &Option<String>,
) -> anyhow::Result<Arc<dyn deepnova_provider::Provider>> {
...
fn resolve_provider_for_task(
    config: &deepnova_config::Config,
    model: &Option<String>,
    task: Option<deepnova_provider::factory::ReasoningEffort>,
) -> anyhow::Result<Arc<dyn deepnova_provider::Provider>> {
    ...
    Ok(deepnova_provider::factory::create_provider_for_task(provider_cfg, task)?.into())
}

/// Build an agent with built-in tools registered.
fn build_agent(
    provider: Arc<dyn deepnova_provider::Provider>,
    _model: Option<&str>,
    config: &deepnova_config::Config,
    max_steps: usize,
) -> anyhow::Result<deepnova_agent::Agent> {
    let workspace_root = std::env::current_dir().unwrap_or_default();
    let security = deepnova_runtime::build_security_context(config, &workspace_root)?;

    let mut agent = deepnova_agent::Agent::new(
        provider,
        if max_steps > 0 {
            max_steps
        } else {
...
async fn stream_events(runner: &dyn Runner, input: RunInput) -> anyhow::Result<()> {
    ...
    while let Some(event) = stream.next().await {
        match event? {
            deepnova_core::RunEvent::TextDelta(text) => {
                print!("{text}");
            }
            deepnova_core::RunEvent::ToolCallStart { id, name } => {
                println!("\n🔧 {name} (call {id})...");
            }
            deepnova_core::RunEvent::ToolCallEnd {
                name: _, arguments, ..
            } => {
                println!("   args: {arguments}");
            }
            deepnova_core::RunEvent::Usage(u) => {
                info!("tokens: {}/{}", u.prompt_tokens, u.completion_tokens);
            }
            deepnova_core::RunEvent::TurnComplete => {
                println!();
            }
            deepnova_core::RunEvent::Done(output) => {
                println!("\n--- done ---");
...
async fn stream_coordinator(runner: &dyn Runner, input: RunInput) -> anyhow::Result<()> {
    ...
    while let Some(event) = stream.next().await {
        match event? {
            ...
            deepnova_core::RunEvent::ToolResult { call_id, result } => {
            ...
            deepnova_core::RunEvent::ReasoningDelta { text, .. } => {
