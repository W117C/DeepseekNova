...
use deepnova_core::runner::{RunEvent, RunEventStream, RunInput, RunOutput, Runner};
use deepnova_serve::{ChatRequest, Server};
...
impl Runner for ServeMockRunner {
    async fn run_stream(&self, _input: RunInput) -> anyhow::Result<RunEventStream> {
        let events: Vec<anyhow::Result<RunEvent>> = vec![
            ...
            Ok(RunEvent::Usage(deepnova_core::chunk::Usage::default())),
            Ok(RunEvent::Done(RunOutput {
                text: "Hello World".to_string(),
                tool_calls: vec![],
                usage: Some(deepnova_core::chunk::Usage {
                    prompt_tokens: 10,
                    completion_tokens: 5,
                    total_tokens: 15,
                    cache_hit_tokens: 0,
                ...
                axum::routing::post(
                    |axum::extract::State(state): axum::extract::State<Arc<Server>>,
                     axum::Json(req): axum::Json<ChatRequest>| async move {
                        let input = deepnova_core::runner::RunInput {
                            prompt: req.prompt,
