//! deepnova-config — layered configuration with TOML loading and merge semantics.
//!
//! Precedence (lowest to highest):
//!   1. Hard-coded defaults
//!   2. `~/.deepnova/config.toml`  (user)
//!   3. `./deepnova.toml`          (project)
...
impl Config {
    ...
    pub fn load() -> anyhow::Result<Self> {
        ...
        // Layer 1: user-global config (~/.deepnova/config.toml)
        ...
        // Layer 2: project-local config (./deepnova.toml)
        let project_path = PathBuf::from("deepnova.toml");
...
fn user_config_path() -> Option<PathBuf> {
    dirs::home_dir().map(|home| home.join(".deepnova").join("config.toml"))
}
