//! Shared test utilities for deepnova-agent integration tests.

use deepnova_core::chunk::Chunk;
use deepnova_core::{Message, Role, RunInput, RunOutput};
use deepnova_provider::{Provider, ValidatedRequest};
...
pub struct MockProvider {
    ...
    tools: HashMap<String, Arc<dyn deepnova_core::Tool>>,
}
...
impl MockProvider {
    ...
    pub fn text(text: impl Into<String>) -> Self {
        Self {
            responses: Mutex::new(vec![vec![
                ...
                Chunk::Usage(deepnova_core::chunk::Usage::default()),
                Chunk::Done,
            ]]),
            tools: HashMap::new(),
        }
    ...
    pub fn with_tools(mut self, tools: Vec<Arc<dyn deepnova_core::Tool>>) -> Self {
        for t in tools {
...
pub struct MockRunner {
    events: Vec<deepnova_core::RunEvent>,
}

impl MockRunner {
    pub fn new(events: Vec<deepnova_core::RunEvent>) -> Self {
        Self { events }
    }
    ...
    pub fn text(text: &str) -> Self {
        Self {
            events: vec![
                deepnova_core::RunEvent::TextDelta(text.to_string()),
                deepnova_core::RunEvent::Done(RunOutput {
                    text: text.to_string(),
                    tool_calls: vec![],
                    usage: None,
                }),
...
#[async_trait::async_trait]
impl deepnova_core::Runner for MockRunner {
    async fn run_stream(&self, _input: RunInput) -> anyhow::Result<deepnova_core::RunEventStream> {
        let events: Vec<anyhow::Result<deepnova_core::RunEvent>> =
