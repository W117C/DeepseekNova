...
async fn run_plan_mode(
    ...
    let validated = deepnova_provider::ValidatedRequest::new(&messages, &[]);
    ...
    while let Some(chunk) = stream.next().await {
        match chunk? {
            deepnova_core::chunk::Chunk::TextDelta(delta) => {
                plan_text.push_str(&delta);
                tx.send(Ok(RunEvent::TextDelta(delta))).await.ok();
            }
            deepnova_core::chunk::Chunk::ReasoningDelta { text, signature } => {
                tx.send(Ok(RunEvent::ReasoningDelta { text, signature }))
                    .await
                    .ok();
            }
            deepnova_core::chunk::Chunk::Usage(u) => {
                tx.send(Ok(RunEvent::Usage(u))).await.ok();
            }
            deepnova_core::chunk::Chunk::Done => {}
...
fn action_label(action: &deepnova_core::graph::Action) -> String {
    match action {
        deepnova_core::graph::Action::Think { .. } => "Think".to_string(),
        deepnova_core::graph::Action::CallTool { tool, .. } => format!("CallTool({tool})"),
        deepnova_core::graph::Action::Observe { .. } => "Observe".to_string(),
        deepnova_core::graph::Action::Reflect { .. } => "Reflect".to_string(),
        deepnova_core::graph::Action::Delegate { sub_agent, .. } => {
            format!("Delegate({sub_agent})")
        }
        deepnova_core::graph::Action::Parallel(_) => "Parallel".to_string(),
        deepnova_core::graph::Action::Conditional { .. } => "Conditional".to_string(),
    }
}
...
mod tests {
    use super::*;
    use deepnova_core::chunk::{Chunk, Usage};
    use deepnova_core::graph::{Action, ExecutionNode};
    use deepnova_provider::Provider;
    use tokio_stream::StreamExt;
    ...
    impl Provider for MockProvider {
        async fn generate(
            &self,
            _validated: deepnova_provider::ValidatedRequest<'_>,
        ...
        async fn stream(
            &self,
            _validated: deepnova_provider::ValidatedRequest<'_>,
        ) -> anyhow::Result<deepnova_core::chunk::ChunkStream> {
