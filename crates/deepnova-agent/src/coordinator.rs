...
use deepnova_core::executor::{
    DelegateCallback, GraphExecutor, ReflectCallback, ReflectResult, ThinkCallback, ToolCallback,
};
use deepnova_core::graph::{Action, ExecutionGraph, ExecutionNode};
use deepnova_core::tool::ToolContext;
use deepnova_core::{Message, Role, RunEvent, RunEventStream, RunInput, RunOutput, Runner, Tool};
...
/// Pause structure used by DeepSeek-DeepNova's TASK_CONTRACT.
...
async fn run_coordinator(
    ...
    let validated =
        deepnova_provider::ValidatedRequest::new(&plan_messages, &[]).map_err(|violations| {
            anyhow::anyhow!(
    ...
    for (node_id, output) in &result.node_outputs {
        match output {
            deepnova_core::graph::NodeOutput::Text(t) => {
                let chunk = format!("[{node_id}]: {t}\n\n");
                combined.push_str(&chunk);
                tx.send(Ok(RunEvent::TextDelta(chunk))).await.ok();
            }
            deepnova_core::graph::NodeOutput::ToolResult(r) => {
            ...
            deepnova_core::graph::NodeOutput::Error(e) => {
...
impl ThinkCallback for CoordinatorCallbacks {
    async fn think(&self, prompt: &str) -> anyhow::Result<String> {
        ...
        let validated =
            deepnova_provider::ValidatedRequest::new(&messages, &[]).map_err(|violations| {
                for v in &violations {
