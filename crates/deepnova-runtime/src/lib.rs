...
//!
//! Wires together all DeepNova subsystems: registry, context, event bus,
//! permission, security, and LLM provider into a ready-to-use agent runtime.

use std::path::PathBuf;
use std::sync::Arc;

use deepnova_config::Config;
use deepnova_context::ContextProvider;
use deepnova_core::registry::RegistryHub;
use deepnova_core::runner::{RunEventStream, RunInput, Runner};
use deepnova_event::EventBus;
use deepnova_permission::{Decision, PermissionGate, Policy};
use deepnova_security::audit::TracingAuditLogger;
use deepnova_security::capability::Capability;
use deepnova_security::context::SecurityContext;
use deepnova_security::limits::ResourceLimits;
use deepnova_security::policy::SecurityPolicy;
...
impl Runtime {
    ...
    pub async fn run(
        ...
        self.events
            .publish(deepnova_event::AgentEvent::ModelStarted {
    ...
    pub fn check_permission(&self, tool: &dyn deepnova_core::Tool, args: &str) -> Decision {
...
fn build_permission_gate(config: &Config) -> PermissionGate {
    ...
    for rule in &config.permissions.rules {
        let r = if let Some(ref subject) = rule.subject {
            deepnova_permission::Rule::with_subject(&rule.tool, subject)
        } else {
            deepnova_permission::Rule::new(&rule.tool)
        };

        match rule.mode {
            deepnova_config::PermissionMode::Allow => allow.push(r),
            deepnova_config::PermissionMode::Ask => ask.push(r),
            deepnova_config::PermissionMode::Deny => deny.push(r),
        }
    }

    let mode = match config.permissions.default_mode {
        deepnova_config::PermissionMode::Allow => Decision::Allow,
        deepnova_config::PermissionMode::Ask => Decision::Ask,
        deepnova_config::PermissionMode::Deny => Decision::Deny,
    };
...
mod tests {
    use super::*;
    use deepnova_config::Config;
    use deepnova_context::ContextEngine;
    use deepnova_security::capability::Capability;
    ...
    fn build_security_context_default_grants_all_capabilities() {
        ...
        let root =
            std::env::temp_dir().join(format!("deepnova-sec-default-{}", std::process::id()));
    ...
    fn build_security_context_honors_disabled_capabilities_and_lists() {
        ...
        let root =
            std::env::temp_dir().join(format!("deepnova-sec-restricted-{}", std::process::id()));
        std::fs::create_dir_all(&root).unwrap();
    ...
    fn build_security_context_applies_resource_limits() {
        ...
        let root = std::env::temp_dir().join(format!("deepnova-sec-limits-{}", std::process::id()));
        std::fs::create_dir_all(&root).unwrap();
    ...
    fn runtime_builds_with_default_config() {
        ...
        let dir = std::env::temp_dir().join(format!("deepnova-rt-test-{}", std::process::id()));
